...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from OSE Github (http://github.com/OSE-Lollipop)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/OSE-Lollipop/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'OSE-Lollipop/%s already exists' % (repo_name)
            else:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for OSE-Lollipop/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: OSE-Lollipop/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "OSE-Lollipop/%s" % repo_name, "revision": "lp5.0" })
