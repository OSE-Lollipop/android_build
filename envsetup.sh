...
- oseremote: Add a git remote for matching ose repository.
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the OSE-Lollipop github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.ose\.version/s/.*=//p' $OUT/system/build.prop`
...
function oseremote()
{
    ...
    git remote add ose git@github.com:OSE-Lollipop/$PFX$PROJECT
    echo "Remote 'ose' created"
    fi
}
